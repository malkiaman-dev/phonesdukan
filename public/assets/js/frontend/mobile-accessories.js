(function () {
    ...
    document.addEventListener('DOMContentLoaded', function () {
        var faqItems = Array.from(document.querySelectorAll('.ma-faq-item'));
        if (!faqItems.length) return;

        function closeItem(item) {
            var btn = item.querySelector('.ma-faq-question');
            var answer = item.querySelector('.ma-faq-answer');
    ...
    /* ── Accessories ads: CSS-first + JS fallback ───────── */
    (function () {
        ...
        function getIframeMetrics(ins) {
            ...
            return { height: readSize(height), width: readSize(width) };
        }
        ...
        function evaluateAd(ins) {
            ...
            if (status === 'filled') {
                ...
                setTimeout(function () {
                    if (isTinyAd(ins)) collapseContainer(ins);
                    else showContainer(ins);
                }, 250);
                return;
            }
            if (isTinyAd(ins)) {
