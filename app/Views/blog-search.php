        ...
        <?php else: ?>
            <div class="posts-grid">
                <?php foreach ($posts as $post): ?>
                    <a href="/blog/<?= htmlspecialchars($post['category_slug']) ?>/<?= htmlspecialchars($post['slug']) ?>" class="post-link">
                        <article class="post">
        ...
        <?php if ($total_pages > 1): ?>
            <nav class="pagination">
                <?php for ($i = 1; $i <= $total_pages; $i++): ?>
                    <a href="/blog-search?paged=<?= $i ?>&s=<?= urlencode($search_term) ?>" class="<?= $i === $current_page ? 'active' : '' ?>"><?= $i ?></a>
                <?php endfor; ?>
            </nav>
        <?php endif; ?>
    </div>
