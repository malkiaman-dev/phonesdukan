...
<!-- === VARIATION SWATCHES === -->
<?php
$showProductVariations = !empty($isVariableProduct) && !empty($productVariations);
$productVariationsHtml = '';
if ($showProductVariations):
    ob_start();
    $usedTypeIds = [];
...
<?php
    $productVariationsHtml = ob_get_clean();
endif;

if ($isComingSoon) {
    include __DIR__ . '/partials/coming-soon-notice.php';
    if ($showProductVariations) {
        echo $productVariationsHtml;
    }
} elseif ($showProductVariations) {
    echo $productVariationsHtml;
}
?>
...
if ($cartFormCondition): ?>
    ...
    <?php endif; ?>
<?php else: ?>
    <?php if (!$isComingSoon && $productAvailability !== 'instock'): ?>
