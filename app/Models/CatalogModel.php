...
class CatalogModel
{
    ...
    {
        $stmt = $this->db->query('SELECT brand_id, brand_name, slug FROM brands ORDER BY brand_name ASC');
        return $stmt->fetchAll(PDO::FETCH_ASSOC) ?: [];
    }

    public function getBrandById(int $id): ?array
    {
        $stmt = $this->db->prepare('SELECT brand_id, brand_name, slug FROM brands WHERE brand_id = ? LIMIT 1');
        $stmt->execute([$id]);
    ...
    public function addBrand(string $name, string $slug): bool
    {
        $stmt = $this->db->prepare('INSERT INTO brands (brand_name, slug) VALUES (?, ?)');
        return $stmt->execute([$name, $slug]);
    }

    public function updateBrand(int $id, string $name, string $slug): bool
    {
        $stmt = $this->db->prepare('UPDATE brands SET brand_name = ?, slug = ? WHERE brand_id = ?');
    ...
    {
        $sql = 'SELECT category_id, category_name, slug, status, sort_order
                FROM categories
    ...
    public function getCategoryById(int $id): ?array
    {
        $stmt = $this->db->prepare(
            'SELECT category_id, category_name, slug, status, sort_order, parent_id
    ...
    public function addCategory(string $name, string $slug, ?int $parentId = null, int $sortOrder = 0, int $status = 1): bool
    {
        $stmt = $this->db->prepare(
            'INSERT INTO categories (category_name, slug, parent_id, sort_order, status)
             VALUES (?, ?, ?, ?, ?)'
        );
        return $stmt->execute([
            $name,
    ...
    public function updateCategory(int $id, string $name, string $slug, ?int $parentId, int $sortOrder, int $status): bool
    {
        $stmt = $this->db->prepare(
            'UPDATE categories
        ...
        return $stmt->execute([$name, $slug, $parentId ?: null, $sortOrder, $status, $id]);
    }
